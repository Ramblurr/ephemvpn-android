...
import com.binaryelysium.ephemvpn.support.Utils;
...
public class ScriptActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // "External storage not mounted", Toast.LENGTH_LONG);
        // toast.show();
        // return;
        // }
    ...
    final Handler installerHandler = new Handler() {
        ...
        public void handleMessage(Message message) {
            ...
            if (bundle.containsKey("showProgressDialog")) {
                myProgressDialog = ProgressDialog.show(ScriptActivity.this, "Installing",
                        "Loading", true);
                ...
                Toast toast = Toast.makeText(getApplicationContext(), "Install Succeed",
                        Toast.LENGTH_LONG);
                toast.show();
            } else if (bundle.containsKey("installFailed")) {
                Toast toast = Toast.makeText(getApplicationContext(),
                        "Install Failed. Please check logs.", Toast.LENGTH_LONG);
                toast.show();
            }
        }
    };
    ...
    private boolean isInstallNeeded() {
        File testedFile = new File(this.getFilesDir().getAbsolutePath() + "/"
        ...
        for (int i = 0; i < t.length; i++) {
            try {
                ...
                if (sFileName.endsWith(GlobalConstants.PYTHON_PROJECT_ZIP_NAME)) {
                    succeed &= Utils.unzip(content, this.getFilesDir().getAbsolutePath() + "/",
                            true);
                }
                ...
                // python extras -> /sdcard/com.android.python27/extras/python
                    ...
                    Utils.createDirectoryOnExternalStorage(this.getPackageName() + "/" + "extras"
                            + "/" + "tmp");
                    succeed &= Utils.unzip(content, Environment.getExternalStorageDirectory()
                            .getAbsolutePath() + "/" + this.getPackageName() + "/extras/", true);
                }
